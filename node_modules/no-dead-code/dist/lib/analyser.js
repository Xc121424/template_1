...
const findExportedIdentifiers = (file) => {
    const exports = [];
    (0, types_1.traverse)(file.ast, {
        enter(path) {
            switch (path.type) {
                // ES module exports
                // export default foo
                ...
                case "ExportNamedDeclaration":
                    if (path.declaration?.type === "VariableDeclaration") {
                        exports.push(...path.declaration.declarations.map(decl => decl.id.type === "Identifier" ? decl.id.name : undefined).filter(Boolean));
                    }
                    if (path.specifiers) {
                        exports.push(...path.specifiers.map(specifier => specifier.exported.type === "Identifier" ? specifier.exported.name : undefined).filter(Boolean));
                    }
                    break;
                ...
                case "ExpressionStatement":
                    if (path.expression.type !== "AssignmentExpression")
                        return;
                    if (path.expression.left.type !== "MemberExpression")
                        return;
                    if (path.expression.left.object.type !== "Identifier" || path.expression.left.object.name !== "module")
                        return;
...
const getRequire = (path) => {
    if (path.type !== "CallExpression")
        return;
    if (path.callee.type !== "Identifier")
        return;
    if (path.callee.name !== "require")
        return;
    if (path.arguments.length !== 1)
        return;
    if (path.arguments[0].type !== "StringLiteral")
        return;
...
const findImportedIdentifiers = (file) => {
    const imports = [];
    (0, types_1.traverse)(file.ast, {
        enter(path) {
            switch (path.type) {
...
const resolvePathAlias = (paths, pathSegments) => {
    let resolveds = [];
    Object.keys(paths).forEach(alias => {
        const aliasSegments = splitPath(alias);
        const tempPathSegments = [...pathSegments];
        let wildCard; // Only one wildcard allowed (tsdocs)
        let specifity = 0;
        ...
        if (aliasSegments.length === 0) {
            ...
            if (resolvedPaths) {
                resolveds.push(...resolvedPaths.map(resolvedPaths => ({
...
const analyse = (root, standardLib, allowDevDeps = false) => {
    const allSourceFiles = [];
    const walkFiles = (file, path = []) => {
        if (file.type === "file") {
        ...
        if (segment === undefined) {
            if (file.type === "file") {
                ...
                if (indexFile?.type === "file") {
                    ...
                    (0, logger_js_1.warn)(`Could not find index file for ${file.name}`, importInfo.from);
                    return false;
                }
            }
        }
            ...
            if (file.parent) {
                ...
                (0, logger_js_1.warn)(`Could not find parent for ${file.name}`);
                return false;
            }
        }
        else {
            if (file.type === "file") {
                (0, logger_js_1.warn)(`Could not find child ${segment} for ${file.name} (is not a directory)`);
                return false;
            }
            ...
            if (rest.length === 0 && childFile) {
                ...
                (0, logger_js_1.warn)(`Could not find child ${segment} for ${file.name} (${importInfo.from})`);
                return false;
            }
        }
    };
    ...
    const walkImports = (file, path = []) => {
        if (file.type === "file") {
            file.moduleInfo?.imports.forEach(importInfo => {
                ...
                if (!file.parent) {
                    (0, logger_js_1.warn)(`Could not find parent for ${file.name}. Outside scan range.`);
                    return;
                }

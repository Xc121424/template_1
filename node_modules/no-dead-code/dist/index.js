...
var warn = (...content) => {
  console.log(import_chalk.default.gray(content));
};

// src/lib/analyser.ts
var findExportedIdentifiers = (file) => {
  const exports2 = [];
  (0, import_types.traverse)(file.ast, {
    enter(path3) {
      switch (path3.type) {
            ...
            path3.expression.right.properties.forEach((prop) => {
              if (prop.type === "ObjectProperty") {
                if (prop.key.type === "Identifier") {
                  exports2.push(prop.key.name);
                } else if (prop.key.type === "StringLiteral") {
                  exports2.push(prop.key.value);
                }
              }
            });
          }
...
var analyse = (root, standardLib, allowDevDeps = false) => {
  ...
  const walkFiles = (file, path3 = []) => {
    if (file.type === "file") {
      const exports2 = findExportedIdentifiers(file);
      const imports = findImportedIdentifiers(file);
      file.moduleInfo = { exports: exports2, unusedExports: [...exports2], imports };
...
var readDir = async (dir, extensions, ignore) => {
  ...
  const rootPath = import_node_path.default.join(cwd, dir);
  const files = await import_node_fs.default.promises.readdir(rootPath);
  const results = await Promise.all(files.filter(
    ...
    if (stats.isDirectory()) {
      ...
      return {
        ...
        name: file,
        path: filePath,
        content: await import_node_fs.default.promises.readFile(filePath, "utf-8")
      };
      ...
      return {
        ...
        name: file,
        path: filePath,
        config: evalJsonObject(await import_node_fs.default.promises.readFile(filePath, "utf-8"))
      };
    } else {
      return null;
    }
...
var parseFile = (file) => {
  const ast = (0, import_parser.parse)(file.content, {
    sourceType: "module",
...
var printUnusedExports = (sourceFiles) => {
  ...
  if (filesWithUnusedExports.length > 0) {
    console.log(import_chalk2.default`{bgRedBright
...
{black >> {bold ${totalCount}} unused exports (suspected)}}
  ...
  filesWithUnusedExports.forEach((file) => {
    const path3 = file.path.split("/").map((segm) => segm).join(import_chalk2.default.gray("/"));
    console.log(import_chalk2.default`{underline ${path3}}:\n {gray Unused exports:}\n ${file.moduleInfo.unusedExports.map((e) => e === "default" ? import_chalk2.default.bold.yellowBright(e) : import_chalk2.default.yellowBright(e)).join(", ")}`);
  });
};

// src/index.ts
import_commander.program.name("no-dead-code").description("Search unused exports in your code").argument("[path]", "the path to begin search from", ".").option("--no-default-ignore", "specify to allow node_modules, .git, dist, build and migrations to be included").option("--no-node-stdlib", "specify to not resolve to node std APIs").option("--no-dev", "specify to not resolve dev dependecies").option("-i, --ignore <ignorePaths...>", "specify paths to ignore").option("-e, --extensions <extensions...>", "specify the extensions to include", ["mjs", "cjs", "js", "ts", "tsx", "jsx"]).parse();
var path2 = import_commander.program.processedArgs[0];
